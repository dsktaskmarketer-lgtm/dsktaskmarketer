...
export const UserWithdrawalsView: React.FC<UserWithdrawalsViewProps> = ({
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    if (amount > availableBal) {
      setError(`Requested amount exceeds your available unlocked balance of ₹${availableBal}`);
      return;
    }
    ...
    try {
      ...
      setSuccessMsg("Withdrawal request submitted successfully! Funds will be reviewed and disbursed to your account.");
    } catch (err: any) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Form Card */}
        <div className="lg:col-span-2 bg-white dark:bg-slate-800/80 rounded-3xl p-6 sm:p-8 border border-slate-200/80 dark:border-slate-700/60 shadow-xs space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-700/60">
            ...
            <div className="text-right text-xs">
              ...
              <div className="font-bold text-slate-700 dark:text-slate-300">₹{minLimit}</div>
            </div>
          </div>
          ...
          <form onSubmit={handleWithdraw} className="space-y-5 text-xs">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g. 500"
                  className="w-full pl-8 pr-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 text-sm font-bold text-slate-900 dark:text-white"
                />
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <Smartphone className={`w-5 h-5 ${method === 'upi' ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-400'}`} />
                  <div>
                ...
                >
                  <Building className={`w-5 h-5 ${method === 'bank_transfer' ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-400'}`} />
                  <div>
            ...
            {method === 'upi' && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 text-xs text-slate-900 dark:text-white font-mono"
                />
              </div>
            )}
              ...
              <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-700">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 text-xs text-slate-900 dark:text-white"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 text-xs text-slate-900 dark:text-white font-mono"
                    />
                  </div>
                  <div>
                    <label className="block font-bold text-slate-800 dark:text-slate-200 mb-1">
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 text-xs text-slate-900 dark:text-white font-mono uppercase"
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-xl text-amber-900 dark:text-amber-200 text-xs space-y-1">
            ...
            <button
              ...
              className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-xs transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
            >
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 text-white rounded-3xl p-6 space-y-4 shadow-sm">
            <h4 className="font-bold text-base">{t('withdrawals.guidelines', 'Payout Guidelines')}</h4>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700/60">
                {withdrawals.map(item => {
                  ...
                  const paymentRef = item.paymentReference || item.transactionReference;
