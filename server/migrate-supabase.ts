...
export async function runSupabaseMigration() {
  ...
  try {
    ...
    await client.query(`
      ...
      DO $$ BEGIN
          CREATE TYPE transaction_type_enum AS ENUM ('task_reward', 'referral_reward', 'withdrawal', 'withdrawal_refund', 'admin_reward', 'manual_reward', 'reversal', 'bonus');
      EXCEPTION WHEN duplicate_object THEN null;
      END $$;
    `);
    ...
    await client.query(`
      ...
      BEGIN
          RETURN (
              ...
              (auth.jwt() ->> 'email') IN ('admin@dsktaskmarketer.com', 'dsktaskmarketer@gmail.com', 'dsabithkumar4@gmail.com', 'honeybezx444@gmail.com') OR
              EXISTS (
      ...
      CREATE POLICY "Public can view active tasks" ON public.tasks
          FOR SELECT USING (active IS TRUE OR active IS NULL OR public.is_admin());
